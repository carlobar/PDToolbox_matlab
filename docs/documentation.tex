...
The toolbox uses a structure with all the parameters required to run the simulations. The following is an example to define a game with one population, three strategies per population. 
%
\begin{lstlisting}
G = struct('n', 3, 'f', @fitness1, 'x0',  [0.2 .7 0.1 ]', 'ode', 'ode113', 'time', 60);
\end{lstlisting}
%
Note that the number of populations and the mass of each population are defined by default to one. The parameters of the population game are defined in Table \ref{tab:req_a}. The simulation can be started by executing
%
\begin{lstlisting}
 G.run()
\end{lstlisting}
...
The following structure is used to define a population game with small number of agents per population 
%
\begin{lstlisting}
